...
describe('Deserialize', () => {
    ...
    for (const legacy of [true, false]) {
        const options = {
            backwardsCompatibility: legacy
        }

        describe(`Deserializing using ${legacy ? 'legacy' : 'new'} structure`, function () {
            describe('Documents with Timestamps', () => {
                it('should deserialize a document containing a Timestamp', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.Timestamp, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
            ...
            describe('Documents with GeoPoints', () => {
                it('should deserialize a document containing a simple GeoPoint', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.GeoPoint, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
                ...
                it('should deserialize a document containing a GeoPoint with floats', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.GeoPointFloat, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
                ...
                it('should deserialize a document containing a GeoPoint with negative floats', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.GeoPointFloatNegative, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
            ...
            describe('Documents with DocumentReferences', () => {
                it('should deserialize a document containing a DocumentReference', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.DocumentReference, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
            ...
            describe('Documents with multiple/nested values', () => {
                it('should deserialize a document containing multiple properties', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.Multiple, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
                ...
                it('should deserialize a document containing nested properties', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.Nested, legacy)
                    const deserializedDocumentSnapshot = deserializeDocumentSnapshot(
                        stringToDeserialize,
                        db,
                        options,
            ...
            describe('Queries with multiple documents', () => {
                it('should deserialize an array of DocumentSnapshots', async () => {
                    const stringToDeserialize = getDeserializationTestString(DeserializationTestString.Query, legacy)
